...
`Ctrl-Tab` and `Ctrl-Shift-Tab` are implemented with Alacritty keyboard bindings that emit xterm `modifyOtherKeys` sequences. tmux parses those as `C-Tab` / `C-S-Tab`. This avoids macOS `Cmd-*`, `Option/Alt-*`, and `Ctrl-Arrow` conflicts while still providing prefix-free tmux window navigation.
